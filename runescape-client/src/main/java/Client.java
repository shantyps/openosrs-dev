...
public final class Client extends GameEngine implements Usernamed, OAuthTokens {
	...
	static {
		...
		cameraShaking = new boolean[5]; // L: 604
		cameraShakeIntensity = new int[5]; // L: 605
		cameraMoveIntensity = new int[5]; // L: 606
		cameraShakeSpeed = new int[5]; // L: 607
		cameraShakeCycle = new int[5]; // L: 608
		field690 = 256; // L: 609
		field707 = 205; // L: 610
			...
			if (--field517 + 1 <= 0) { // L: 2019
				try {
					if (js5ConnectState == 0) { // L: 2021
						ScriptEvent.js5SocketTask = GameEngine.taskHandler.newSocketTask(MoveSpeed.worldHost, class129.currentPort); // L: 2022
		...
		try {
			...
			if (loginState == 1) { // L: 2884
				if (class334.socketTask == null) { // L: 2885
					class334.socketTask = GameEngine.taskHandler.newSocketTask(MoveSpeed.worldHost, class129.currentPort); // L: 2886
				}
				...
				if (loginState != 16) { // L: 3237
					...
					if (((AbstractSocket)var1).available() >= packetWriter.serverPacketLength) { // L: 3238
						...
						for (var34 = 0; var34 < 2048; ++var34) { // L: 3281
							...
							Players.field1246[var34] = MoveSpeed.field2123; // L: 3283
						}
													...
													do {
														...
														if (var40 == null) { // L: 4011
															...
															if (field724) { // L: 4246
																...
																var4 = class167.cameraMoveToX * 128 + 64; // L: 4248
																var5 = VarpDefinition.cameraMoveToY * 128 + 64; // L: 4249
																var6 = FaceNormal.getTileHeight(var4, var5, class20.Client_plane) - UserComparator9.cameraMoveToHeight; // L: 4250
																class1.method12(var4, var6, var5); // L: 4251
																var4 = PlayerComposition.cameraLookAtX * 128 + 64; // L: 4252
																var5 = MusicPatchNode.cameraLookAtY * 128 + 64; // L: 4253
																var6 = FaceNormal.getTileHeight(var4, var5, class20.Client_plane) - ModeWhere.cameraLookAtHeight; // L: 4254
		...
		if (var2 == null) { // L: 6474
			...
			try {
				...
				if (ServerPacket.field3021 == var1.serverPacket) { // L: 6678
					...
					for (; var6-- > 0; var54.field1014 = true) { // L: 6689 6699
						...
						if (var54 == null) { // L: 6698
							var54 = ChatChannel.ifOpenSub(var7, var26, var9);
						}
					}
				...
				if (ServerPacket.field3027 == var1.serverPacket && isCameraLocked) { // L: 6760 6761
					...
					Renderable.cameraMoveToSpeed = var3.readUnsignedByte(); // L: 6763
					ItemComposition.cameraMoveToAcceleration = var3.readUnsignedByte(); // L: 6764
					WorldMapSection1.cameraLookAtSpeed = var3.readUnsignedByte(); // L: 6765
					ChatChannel.cameraLookAtAcceleration = var3.readUnsignedByte(); // L: 6766
				...
				if (ServerPacket.field2942 == var1.serverPacket) { // L: 7018
					...
					PlayerComposition.cameraLookAtX = var3.readUnsignedByte(); // L: 7021
					MusicPatchNode.cameraLookAtY = var3.readUnsignedByte(); // L: 7022
					ModeWhere.cameraLookAtHeight = var3.readUnsignedShort(); // L: 7023
					WorldMapSection1.cameraLookAtSpeed = var3.readUnsignedByte(); // L: 7024
					ChatChannel.cameraLookAtAcceleration = var3.readUnsignedByte(); // L: 7025
					if (ChatChannel.cameraLookAtAcceleration >= 100) { // L: 7026
						var17 = PlayerComposition.cameraLookAtX * 128 + 64; // L: 7027
						var5 = MusicPatchNode.cameraLookAtY * 128 + 64; // L: 7028
						var6 = FaceNormal.getTileHeight(var17, var5, class20.Client_plane) - ModeWhere.cameraLookAtHeight; // L: 7029
				...
				if (ServerPacket.field3007 == var1.serverPacket) { // L: 7042
					...
					class167.cameraMoveToX = var3.readUnsignedByte(); // L: 7045
					VarpDefinition.cameraMoveToY = var3.readUnsignedByte(); // L: 7046
					UserComparator9.cameraMoveToHeight = var3.readUnsignedShort(); // L: 7047
					Renderable.cameraMoveToSpeed = var3.readUnsignedByte(); // L: 7048
					ItemComposition.cameraMoveToAcceleration = var3.readUnsignedByte(); // L: 7049
					if (ItemComposition.cameraMoveToAcceleration >= 100) { // L: 7050
						UserComparator7.cameraX = class167.cameraMoveToX * 128 + 64; // L: 7051
						UserComparator10.cameraZ = VarpDefinition.cameraMoveToY * 128 + 64; // L: 7052
						AbstractByteArrayCopier.cameraY = FaceNormal.getTileHeight(UserComparator7.cameraX, UserComparator10.cameraZ, class20.Client_plane) - UserComparator9.cameraMoveToHeight; // L: 7053
					}
				...
				if (ServerPacket.field2983 == var1.serverPacket) { // L: 7058
					...
					ChatChannel.ifOpenSub(var6, var17, var5); // L: 7064
				...
				if (ServerPacket.field2963 == var1.serverPacket) { // L: 7261
					...
					cameraShaking[var17] = true; // L: 7266
					cameraShakeIntensity[var17] = var5; // L: 7267
					cameraMoveIntensity[var17] = var6; // L: 7268
					cameraShakeSpeed[var17] = var7; // L: 7269
					cameraShakeCycle[var17] = 0; // L: 7270
				...
				if (ServerPacket.field2953 == var1.serverPacket) { // L: 7332
					...
					var25.modelRotation = var6 + (var5 << 16); // L: 7337
				...
				if (ServerPacket.field3003 == var1.serverPacket) { // L: 7587
					...
					class315.playJingle(var5, var17); // L: 7591
	...
	public final void init() {
		try {
			if (this.checkHost()) { // L: 666
				...
				MoveSpeed.worldHost = this.getCodeBase().getHost(); // L: 785
